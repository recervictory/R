# plotting/gene_trends.R
# Smoothed gene expression trend plots along a cell ordering axis.
...
# Plot smoothed expression trends for a set of genes across all cells,
# optionally sorted by cluster and an average of sort_by_gene features.
# Returns a patchwork of the expression line plot and a cluster annotation bar.
plot_gene_trends <- function(seurat_obj, gene_list,
                             sort_by_gene = NULL,
                             bin_size = 50,
                             colors = NULL,
                             cluster_colors = NULL,
  ...
  expr_df <- expr_df %>%
    dplyr::mutate(sort_by_gene_avg = rowMeans(dplyr::across(dplyr::all_of(sort_by_gene)), na.rm = TRUE))

  if (log_transform) {
    if (verbose) message("[Step 4] Applying log1p transformation to gene_list...")
    expr_df <- expr_df %>% dplyr::mutate(dplyr::across(dplyr::all_of(gene_list), log1p))
  }
  ...
  expr_df <- expr_df %>%
    ...
    dplyr::filter(sort_by_gene_avg >= sort_by_gene_cutoff)

  if (verbose) message("[Step 6] Sorting cells...")
  expr_df <- if (sort_by_annotation) {
    expr_df %>%
      dplyr::mutate(cluster = as.factor(cluster)) %>%
      dplyr::arrange(cluster, sort_by_gene_avg)
  } else {
    expr_df %>% dplyr::arrange(sort_by_gene_avg)
  }

  expr_df <- expr_df %>%
    dplyr::mutate(
      cell_order = dplyr::row_number(),
      bin = dplyr::ntile(sort_by_gene_avg, bin_size)
    )

  if (verbose) message("[Step 7] Calculating binned means...")
  binned_means <- expr_df %>%
    dplyr::group_by(bin) %>%
    dplyr::summarise(dplyr::across(dplyr::all_of(gene_list), mean), .groups = "drop") %>%
    dplyr::mutate(cell_order = bin * (max(expr_df$cell_order) / bin_size))

  plot_df <- binned_means %>%
    tidyr::pivot_longer(cols = dplyr::all_of(gene_list), names_to = "gene", values_to = "expression")
  ...
  main_plot <- ggplot(plot_df, aes(x = cell_order, y = expression, color = gene)) +
    geom_line(size = 1.2) +
    geom_text(
      data = plot_df %>% dplyr::group_by(gene) %>% dplyr::slice_tail(n = 1),
  ...
  if (is.null(cluster_colors)) {
    ...
    cluster_palette  <- colorRampPalette(RColorBrewer::brewer.pal(12, "Set3"))(length(unique_clusters))
